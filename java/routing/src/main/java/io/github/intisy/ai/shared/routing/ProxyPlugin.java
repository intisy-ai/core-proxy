 ...
 * the proxy-side mirror of {@link Provider}. Where a {@link Provider} represents an upstream account
 * provider (claude, antigravity, …) that serves requests, a {@code ProxyPlugin} represents a host app
 * whose traffic the {@code :34567} proxy fronts (claude-code, opencode, …) and declares only how that
 * proxy should route.
 *
 ...
 *
 * <p>Unlike {@link Provider}, a {@code ProxyPlugin} is not a {@link ProxyHandler}: a proxy does not
 * itself serve upstream requests (the core-proxy {@link io.github.intisy.ai.shared.logic.Router} plus
 * the installed {@link Provider}s do). A {@code ProxyPlugin} is purely declarative: it names the proxy
 * and supplies the {@link RoutingProfile} that configures a proxy instance for it.
 */
